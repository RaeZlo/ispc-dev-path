## Resumen Didáctico sobre Diseño de Bases de Datos
...
*   **Primera Forma Normal (1FN):** Requiere que cada tabla tenga una **clave primaria única**, **no se permitan valores repetidos en las columnas**, y que **cada columna tenga un solo valor atómico en cada fila**. El ejemplo de la tabla "ventas" que se corrige creando "detalle\_ventas" ilustra cómo eliminar la repetición de productos en una misma venta para cumplir con la 1FN.

*   **Segunda Forma Normal (2FN):** Establece que **cada atributo no clave** de una tabla debe **depender funcionalmente de la clave primaria completa**. Esto es relevante cuando una tabla tiene **múltiples claves primarias** (clave primaria compuesta), donde cada atributo debe depender de todas las claves primarias y no solo de una parte. El ejemplo de la tabla "Pedidos" con una clave primaria compuesta ("numero\_pedido", "fecha\_pedido") y el atributo "codigo\_cliente" que solo depende de "numero\_pedido", muestra la necesidad de dividir la tabla en "Pedidos" y "Clientes" para cumplir con la 2FN.

*   **Tercera Forma Normal (3FN):** Se enfoca en la **eliminación de las dependencias transitivas**. Una dependencia transitiva ocurre cuando un atributo depende de otro atributo que no es la clave primaria. Para aplicar la 3FN, se debe asegurar que **cada atributo no clave dependa solo de la clave primaria** de la tabla o de otros atributos clave. El ejemplo de la tabla "ventas" donde "nombre\_producto" y "cantidad" dependen de "id\_producto" (un atributo no clave) en lugar de la clave primaria "id\_venta", se corrige creando una tabla "Productos" separada.
